...
let simpler_sum xs = List.fold ~f: ( + ) ~init:0 xs
let simpler_product xs = List.fold  ~f: ( * ) ~init:1 xs
...
let range from to_ =
  if from > to_ then []
    ...
    let size = (( + ) (abs from) to_) in
    List.init size ~f:(fun i -> from + i)
  else List.init (to_ - 1) ~f:(fun i -> from + i)
...
let print_int_list xs =
  List.iter ~f:(fun num -> Stdio.printf "%d" num) xs
